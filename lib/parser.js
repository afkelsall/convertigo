...
(function () {
  ...
  // Thousands-separated (e.g. 1,800 or 1,000,000) must come before plain decimal to prevent
  // the plain pattern matching just "1" and leaving ",800" unmatched.
  const THOUSANDS = '\\d{1,3}(?:,\\d{3})+(?:\\.\\d+)?';
  const NUMBER = '(\\d+\\s+\\d+\\/\\d+|\\d+\\/\\d+|' + THOUSANDS + '|\\d+(?:[.,]\\d+)?)';
  ...
  const UNIT_REGEX = new RegExp(
    '(?<![$€£¥\\d.A-Za-z])' + NUMBER + '[-\\s]*(?:(' + unitPattern + ')\\b|(' + symbolPattern + ')([A-Za-z]?))',
    'gi'
  );

  // Matches a range: N-M unit (e.g. "2-3in", "2-3 inches")
  const SIMPLE_NUM = '(' + THOUSANDS + '|\\d+(?:[.,]\\d+)?)';
  const RANGE_REGEX = new RegExp(
    '(?<![$€£¥\\d.A-Za-z])' + SIMPLE_NUM + '-' + SIMPLE_NUM + '\\s*(?:(' + unitPattern + ')\\b|(' + symbolPattern + ')([A-Za-z]?))',
    'gi'
  );

  // Dimension pattern: "13 x 72 inches", "13x72x5 cm", "13 × 72 in"
  const DIMENSION_REGEX = new RegExp(
    '(?<![\\d.A-Za-z])\\d+(?:[.,]\\d+)?(?:\\s*[x×X]\\s*\\d+(?:[.,]\\d+)?)+\\s*(?:(' + unitPattern + ')\\b|(' + symbolPattern + ')([A-Za-z]?))',
    'gi'
  );
  ...
  function parseNumber(str) {
    ...
    // Thousands separator: commas with exactly 3 digits per group (e.g. 1,800 or 1,000,000)
  ...
  function hasLeadingZero(numStr) {
    // Reject "007", "00024" but allow "0", "0.5", "0,5"
    return /^0\d/.test(numStr.trim());
  }

  // "in" is also the English preposition. The bare word "in" (not "inch"/"inches" nor the
  // " symbol) is prepositional — not inches — when it's immediately followed by a number or an
  // article/determiner: "40-50 in 60km zones", "85 in a 70", "1 in 3 people". Reject those.
  // A following word like "long"/"wide"/"of" is left alone ("6 in of snow" stays inches).
  const IN_PREPOSITION_OBJECT = /^\s+(?:\d|(?:an?|the|my|your|his|her|its|our|their|this|that|these|those)\b)/i;
  function isPrepositionalIn(unitStr, text, matchEnd) {
    if (!unitStr || unitStr.toLowerCase() !== 'in') return false;
    // Only the detached word "in" is ambiguous; "40in" glued to the number is always inches.
    const before = text[matchEnd - unitStr.length - 1];
    if (!before || !/[\s-]/.test(before)) return false;
    return IN_PREPOSITION_OBJECT.test(text.slice(matchEnd));
  }
  ...
  function parse(text) {
    ...
    while ((match = RANGE_REGEX.exec(text)) !== null) {
      if (usedPositions.has(match.index)) continue;
      if (hasLeadingZero(match[1]) || hasLeadingZero(match[2])) continue;
      ...
      if (isPrepositionalIn(unitStr, text, match.index + match[0].length)) continue;
    ...
    while ((match = DIMENSION_REGEX.exec(text)) !== null) {
      ...
      if (match[1]) {
        ...
        if (!unit) continue;
        if (isPrepositionalIn(match[1], text, match.index + match[0].length)) continue;
      } else if (match[2]) {
      ...
      if (rawParts.some(p => hasLeadingZero(p))) continue;
    ...
    while ((match = UNIT_REGEX.exec(text)) !== null) {
      if (usedPositions.has(match.index)) continue;
      if (hasLeadingZero(match[1])) continue;
      ...
      if (match[2]) {
        ...
        if (!unit) continue;
        if (isPrepositionalIn(match[2], text, match.index + match[0].length)) continue;
        suffix = '';
      } else if (match[3]) {
