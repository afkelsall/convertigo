...
(function () {
  ...
  // --- Regex 4: number then symbol AFTER (European convention) ---
  // Matches: 40€, 90€, 350£, 1,000€, 5M€ — optional space before the symbol
  const SYMBOL_SUFFIX_REGEX = new RegExp(
    '(?<![,\\d])' + NUM + MULT + '\\s*(' + symbolPattern + ')',
    'gi'
  );
  // Groups: [1]=number, [2]=multiplier, [3]=symbol

  // Resolve a currency symbol to its code ('$' depends on site context)
  function symbolToCurrency(symbol, dollarCurrency) {
    return symbol === '$' ? dollarCurrency : PREFIX_SYMBOLS[symbol];
  }
  ...
  function parse(text, options = {}) {
    ...
    while ((match = PREFIX_REGEX.exec(text)) !== null) {
      ...
      if (suffixCode) {
        ...
        currency = symbolToCurrency(symbol, dollarCurrency);
      }
    ...
    // Pass 4: currency symbol AFTER number (40€, 350£ — European convention)
    SYMBOL_SUFFIX_REGEX.lastIndex = 0;
    while ((match = SYMBOL_SUFFIX_REGEX.exec(text)) !== null) {
      const currency = symbolToCurrency(match[3], dollarCurrency);
      const value = parseAmount(match[1], match[2]);
      if (!currency) continue;
      addResult(value, currency, match[0], match.index, match[2] || null);
    }
