...
(function () {
  ...
  // --- Regex 1: prefix symbol (with optional country-dollar prefix) ---
  // Matches: $15, $95,000, $7.3 billion, €40, £350K, C$300, Can$300, US$2
  ...
  const PREFIX_REGEX = new RegExp(
    '(?<![A-Za-z\\d])(?:(' + countryPattern + '))?(' + symbolPattern + ')\\s*' + NUM + MULT,
    'gi'
  );
  // Groups: [1]=countryPrefix, [2]=symbol, [3]=number, [4]=multiplier
  ...
  function parse(text, options = {}) {
    ...
    function addResult(value, currency, matchStr, matchIndex, multiplier) {
      ...
      if (currency === TARGET_CURRENCY) return;
    ...
    // Pass 1: prefix symbols (with optional country prefix)
